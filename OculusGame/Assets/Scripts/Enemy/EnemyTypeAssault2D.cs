...
public class EnemyTypeAssault2D : MonoBehaviour {
	...
	Vector3 createPos;
	void Awake () {
		target = GameObject.FindGameObjectWithTag ("Player2D").transform; //プレイヤー座標取
	...
	IEnumerator EnemyTypeAssaultNextMove(){
		...
		if (target.position.x  <= movePos.x) {
			//敵の位置を取得し再度プレイヤーの方を向く
			Vector3 targetPos = target.position;
			Vector3 direction = transform.position - targetPos;
			transform.rotation = Quaternion.LookRotation (direction);
		...
		//停止予定位置よりも前にいる場合そのまま直進
		else if(target.position.x > movePos.x){
			move = transform.forward * speed * -3f;
			GetComponent<Rigidbody> ().AddForce (move);
		}
